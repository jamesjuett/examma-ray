...
Let's take a quick look at some of these. First, `content/sample_mc.ts` gives a basic example of defining a [[QuestionSpecification]]. More details at [[specification.ts]]

```typescript
[[include:template/content/sample_mc.ts]]
```

#### **`content/sample_mc.ts`**
```typescript
