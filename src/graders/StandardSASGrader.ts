    ...
    <table class="examma-ray-sas-diff table table-sm">
      ...
      return `
        <tr>
          ...
          <td>${included.length === 0 ? `<pre style="font-style: italic">${rubricItem.required.length === 0 ? CHECK_ICON : RED_X_ICON} (no selection)</pre>` : (included.map(line => `<pre>${rubricItem.required.indexOf(line) !== -1 ? CHECK_ICON : RED_X_ICON} <code class="language-cpp">${(<SASItem>question.response.choices[line]).text.replace(/^\n+|\n+$/g, "")}</code></pre>`).join('<br style="font-size: 0.3rem"/>'))}</td>
          <td>${rubricItem.required.map(line => `<pre><code class="language-cpp">${(<SASItem>question.response.choices[line]).text.replace(/^\n+|\n+$/g, "")}</code></pre>`).join('<br style="font-size: 0.3rem"/>')}</td>
        </tr>`;
    }).join("")}
    </table>`;
  }
